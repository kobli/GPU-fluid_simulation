...
and $\mathbf{f}_{i}^{external}$ may be forces from other objects interacting with the fluid. $\mu$ is the viscosity coefficient and 
...
%\lstset{language=C++,caption=Generování n
%náhodně umístěných bodů uvnitř koule.,frame=single,label=listing:koule
%,captionpos=b,basicstyle= \footnotesize,tabsize=1} 
%\begin{lstlisting}
%\end{lstlisting} 
...
%\begin{figure}[!h]
%\centering
%\includegraphics[width=3in]{test2}
%\caption{Doba běhu aplikace pro testovací sadu 2.}
%\label{fig:test2}
%\end{figure}
...
\bibitem{Articles:Shell}
Michael Shell: \emph{How to Use the IEEEtran \LaTeX Class}. 2007.
